...
describe("findAll", function () {
  ...
  test("works: empty list/zero results", async function () {
    let companies = await Company.findAll({ name: "none" });
    expect(companies).toEqual([]);
  });

  test("bad request for invalid min > max", async function () {
    try {
      await Company.findAll({ minEmployees: 20, maxEmployees: 10 });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
